...
export class DecisionEngine {
  ...
  public static evaluate(userText: string, session: ConversationSession): DecisionResult {
    ...
    // 1b. Check for active takeaway order payment / confirmation
    if (session.metadata?.activeOrder) {
      const ao = session.metadata.activeOrder;
      const isPaymentIntent =
        text.includes('lanjut bayar') ||
        text.includes('bayar sekarang') ||
        text.includes('lanjut ke pembayaran') ||
        text.includes('lanjut pembayaran') ||
        text.includes('pembayaran') ||
        text.includes('checkout') ||
        text.includes('proses pesanan') ||
        text.includes('mau bayar') ||
        (text.includes('bayar') && !text.includes('berapa'));

      if (isPaymentIntent) {
        const items = ao.detailedItems || ao.items || [];
        return {
          intent: 'CONFIRM_ACTION',
          confidence: 0.98,
          extractedEntities: { order: ao },
          recommendedTool: 'create_takeaway_order',
          toolArgs: {
            customerName: session.customerName || 'Pelanggan Takeaway',
            customerPhone: session.customerPhone || '-',
            items: items.map((it: any) => ({
              menuName: it.name || it.menuName,
              quantity: it.quantity || 1,
              notes: it.notes,
            })),
            notes: 'Pesanan Bungkus Take Away via AI Chatbot',
          },
        };
      }
      ...
      if (text.includes('batal') || text.includes('cancel')) {
        return {
          intent: 'CANCEL_ACTION',
          confidence: 0.95,
          extractedEntities: { cancelOrder: true },
        };
      }
    }
