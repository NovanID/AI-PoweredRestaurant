...
export const TOOL_REGISTRY: Record<string, ToolDefinition> = {
  ...
  calculate_order_total: {
    ...
    description: 'Menghitung estimasi total harga pesanan makanan beserta pajak jika pelanggan SUDAH menyebutkan nama menu spesifik dan jumlahnya. DILARANG panggil tool ini jika pelanggan belum menyebut nama menu.',
    parameters: {
      type: 'object',
      properties: {
        items: {
          type: 'array',
          description: 'Daftar item pesanan makanan/minuman',
          items: {
            type: 'object',
            properties: {
              menuName: { type: 'string', description: 'Nama menu makanan/minuman (contoh: Rendang Daging Sapi, Ayam Pop)' },
              menuItemId: { type: 'string', description: 'ID menu jika ada' },
              quantity: { type: 'number', description: 'Jumlah porsi pesanan. WAJIB 1 jika pelanggan tidak menyebut angka porsi.' },
            },
            required: ['menuName'],
          },
        },
  ...
  create_takeaway_order: {
    ...
    description: 'Membuat pesanan bungkus (takeaway) resmi ke sistem dan mengunci rincian pesanan untuk lanjut ke pembayaran kasir / Midtrans. Panggil tool ini saat pelanggan menyetujui total pesanan dan ingin konfirmasi/lanjut bayar.',
    parameters: {
      type: 'object',
      properties: {
        customerName: { type: 'string', description: 'Nama pemesan' },
        ...
        items: {
          ...
          items: {
            type: 'object',
            properties: {
              menuName: { type: 'string', description: 'Nama menu makanan/minuman' },
              quantity: { type: 'number', description: 'Jumlah porsi. WAJIB 1 jika tidak ditentukan angka spesifik.' },
              notes: { type: 'string', description: 'Catatan per item' },
            },
            required: ['menuName'],
          },
        },
